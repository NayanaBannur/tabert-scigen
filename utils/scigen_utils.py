    ...
    with open(data_path, "r") as f:
        for text in tqdm(f.readlines()):
            tokenized = tokenizer.batch_encode_plus(
                [text.strip() + ' </s>'], max_length=max_length, pad_to_max_length=pad_to_max_length,
                return_tensors=return_tensors,  # add_special_tokens=True,
...
def encode_file_sents_unsup_bart(tokenizer, data_path, max_source_length, max_target_length, pad_to_max_length=True,
                                 return_tensors="pt"):
    with open(data_path, "r") as f:
        texts = f.readlines()
        ...
        for text in tqdm(texts_source):
            tokenized = tokenizer.batch_encode_plus(
                [text], max_length=max_source_length, pad_to_max_length=pad_to_max_length,
                return_tensors=return_tensors,
            )
...
def encode_file_sents_unsup_leonardo(tokenizer, data_path, max_source_length, max_target_length, pad_to_max_length=True,
                                     return_tensors="pt"):
...
def encode_file_sents_unsup(tokenizer, data_path, max_source_length, max_target_length, pad_to_max_length=True,
                            return_tensors="pt"):
    def get_extra_token(count):
        TOKEN_EXTRA_ID = '<extra_id_'
        token = TOKEN_EXTRA_ID + str(count) + '>'
...
def encode_file_sent_source(tokenizer, data_path, ids, max_length, pad_to_max_length=True, return_tensors="pt"):
    ...
    with open(data_path, "r") as f:
        ...
        for text in tqdm(texts):
            tokenized = tokenizer.batch_encode_plus(
                ['translate Graph to English: ' + text.strip() + ' </s>'], max_length=max_length,
...
def encode_file_text2graph(tokenizer, data_path, max_length, pad_to_max_length=True, return_tensors="pt"):
    ...
    with open(data_path, "r") as f:
        for text in tqdm(f.readlines()):
            tokenized = tokenizer.batch_encode_plus(
                ['translate English to Graph: ' + text.strip() + ' </s>'], max_length=max_length,
                pad_to_max_length=pad_to_max_length, return_tensors=return_tensors,
            )
            examples.append(tokenized)
    return examples
...
class AgendaGraph2textUnsupBARTDataset(Dataset):
    def __init__(
        ...
        self.source, self.target = encode_file_sents_unsup_bart(tokenizer,
...
class TextUnsupDataset(Dataset):
    def __init__(
        ...
        self.source, self.target = encode_file_sents_unsup_leonardo(tokenizer,
                                                                    os.path.join(data_dir, type_path + ".target"),
                                                                    max_source_length,
                                                                    max_target_length)
